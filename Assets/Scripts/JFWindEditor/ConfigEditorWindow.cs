    ...
    public class ConfigEditorWindow : OdinMenuEditorWindow
    {
        ...
        protected override void OnDestroy()
        {
            if (configCreator != null)
                DestroyImmediate(configCreator.data);
        }
        ...
        protected override OdinMenuTree BuildMenuTree()
        {
            ...
            mainMenu.Add("Config Asset Editor", configCreator);

            Assembly assembly = Assembly.Load("Assembly-CSharp");
            if (configCreator.nameSpace == string.Empty) return mainMenu;
            foreach (DataType dataType in Enum.GetValues(typeof(DataType)))
            {
                // 获取对应的数据类型
                Type screenType = assembly.GetType($"{configCreator.nameSpace}.{dataType.ToString()}");
                if (screenType == null)
                {
                    Debug.LogError($"Can't find screen textDataType {configCreator.nameSpace}.{dataType.ToString()}");
                    break;
                }

                // 添加所有路径下的数据资产，并按名称排序
                var assets =
                    mainMenu.AddAllAssetsAtPath($"Data/{screenType.Name}", $"Assets/Resources/SO/Config/{dataType.ToString()}",
                        screenType);
...
// 创建数据资产的辅助类
    public class ConfigCreator
    {
        ...
        [Header("配置文件名称")] public string dataName;
        ...
        {
            string folderPath = $"Assets/Resources/SO/Config/{dataType}";
